...
```{r}
control = rpart.control(minsplit=1,maxdepth=1)
model3 <- rpart(Buys~Humid, data=train, method="class", control = control)
rpart.plot(model3, type=4, extra = 1, digits=-2)
```

### Model 4 - Split on Windy 
...
model4 <- rpart(Buys~Windy, data=train, method="class", control = control)
rpart.plot(model4, type=4, extra = 1, digits=-2)
```

### Model 5 - Split on Temp, then Outlook 

What about if we use both `Temp` and `Outlook` together to predict `Buys`: 

```{r}
control = rpart.control(minsplit=1,maxdepth=3, cp=-1)
model5 <- rpart(Buys~Temp+Outlook, data=train, method="class", control = control)
rpart.plot(model5, type=4, extra = 1, digits=-2)
```

### Model 6 - Split on Temp, then Outlook 

What about if we use both `Temp` and `Humid` together to predict `Buys`: 

```{r}
control = rpart.control(minsplit=1,maxdepth=3, cp=-1)
model6 <- rpart(Buys~Temp+Humid, data=train, method="class", control = control)
rpart.plot(model6, type=4, extra = 1, digits=-2)
```

Which of these models above looks best and why? It is not always easy to tell actually!!

### Model 7 - All the predictors 

Here is a model that uses all the predictors:

```{r}
control = rpart.control(minsplit=1)
model7 <- rpart(Buys~Temp+Humid+Outlook+Windy, data=train, method="class", control = control)
rpart.plot(model7, type=4, extra = 1, digits=-2)
```
...
```{r}
pred <- predict(model3, newdata = test, type = 'class')
```
