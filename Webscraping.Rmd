...
Then once we do this we pick a site we want to get data from. We want to pull some data from poshmark and compare original prices with the current price, maybe to find excellent bargins. We use the `read_html` method from the rvest package. We will look in the category Jackets and Coats-Blazers, and we will just use the data we find on the first page: 
...
First we get the titles using `html_nodes` and the selector `#tiles-con .title` which wegot from using SelectorGadget: 
